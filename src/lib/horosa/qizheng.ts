...
export type QiZhengResult = {
  natal: NatalChart;
  extras: QiZhengItem[];
  mingdu: { name: string; lon: number; dms: string; xiu: string };
  shendu: { name: string; lon: number; dms: string; xiu: string };
  list: QiZhengItem[];
};

const XIU = ["角", "亢", "氐", "房", "心", "尾", "箕", "斗", "牛", "女", "虚", "危", "室", "壁", "奎", "娄", "胃", "昴", "毕", "觜", "参", "井", "鬼", "柳", "星", "张", "翼", "轸"];
const XIU_W = [12, 9, 16, 5, 6, 18, 10, 24, 7, 11, 10, 16, 17, 9, 16, 12, 15, 11, 16, 1, 9, 30, 2, 13, 6, 17, 19, 17];
...
export function computeQizheng(b: BirthInput): QiZhengResult {
  ...
  const mingdu = { name: "命度", lon: mingIdx * 30 + 15, dms: formatDMS(mingIdx * 30 + 15), xiu: xiuOf(mingIdx * 30 + 15) };
  const shendu = { name: "身度", lon: shenIdx * 30 + 15, dms: formatDMS(shenIdx * 30 + 15), xiu: xiuOf(shenIdx * 30 + 15) };

  const classical = natal.planets
    .filter((p) => !p.modern && p.key !== "SNode")
    .map((p) => item(p.name, p.glyph, p.lon));

  return { natal, extras, mingdu, shendu, list: [...classical, ...extras] };
}
